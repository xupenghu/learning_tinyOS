...
void tTaskInit (tTask *task, void (*entry)(void *), void *param, uint32_t prio,  tTastStack * stack)
{
	...
	*(--stack) = (unsigned long )0x04;
	...
	taskTable[prio] = task;		//将初始化好的任务装入tasktable中
	tbitmap_setbit(&task_prio_bitmap, prio); //将相应优先级的位图置1
...
{
	uint32_t prio;
	prio = tbitmap_get_first_set(&task_prio_bitmap);
...
void tTaskSystemTickHandler(void)
{
	...
	{
		if(taskTable[i]== (tTask *)0)   //如果该任务没有被初始化，那么就直接跳过
		{
			continue;
		}
		...
		{
			tbitmap_setbit(&task_prio_bitmap,i);  //否则就让任务进入就绪态
		}
...
void tTaskDelay (uint32_t delay)
{
	...
	tbitmap_clearbit(&task_prio_bitmap, currentTask->prio);  //任务处于没有就绪状态 等延时时间到了 系统会自动将任务处于就绪状态
...
{
	sched_lock_count = 0;
	task_prio_bitmap.bitmap = 0; //调度优先级位图全部清0
}
