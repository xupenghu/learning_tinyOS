...
tTask tIdleTask;
tTaskStack idleTaskEnv[TINYOS_IDELTASK_STACK_SIZE];





static void check_cpu_usage (void);


static uint32_t enable_cpu_usage_stat;
static uint32_t systick_count;
static uint32_t idle_count;
static uint32_t idle_max_count;
...
void t_task_system_tick_handler(void)
{
	...
	for(node = task_delay_list.head_node.nextnode; node != &(task_delay_list.head_node); node = node->nextnode)
	{
		...
		if (--task->delay_ticks == 0)
		{
			...
			{
				//超时强制从延时列表中删除等待列表
				t_event_remove_task(task, 0, t_error_timeout);
	...
	//定时器任务硬件定时器列表处理和软件定时器列表通知
	t_timer_module_tick_notify();
...
static void check_cpu_usage (void)
{
	...
	if(systick_count == TICKS_PER_SEC)
	{
		idle_max_count = idle_count;
		idle_count  = 0;
		enable_task_sched();
	}
	else if(systick_count % TICKS_PER_SEC == 0)
	{
...
** Returned value       :   无
***********************************************************************************************************/
static void cpu_usage_stat_init (void)
{
	idle_count = 0;
	idle_max_count = 0;
	cpu_usage = 0.0;
...
** Returned value       :   无
***********************************************************************************************************/
static void cpu_usage_sync_with_systick (void)
{
    // 等待与时钟节拍同步
...
int main(void)
{
	...
	t_task_init(&tIdleTask, idleTaskEntry, (void *)0, TINYOS_PRIO_COUNT-1, idleTaskEnv, sizeof(idleTaskEnv));
