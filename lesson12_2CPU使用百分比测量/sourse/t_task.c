...
void t_task_init (tTask *task, void (*entry)(void *), void *param, uint32_t prio,  tTaskStack * stack, uint32_t size)
{
	...
	stack_top = stack + size / sizeof(tTaskStack) ;
	
	*(--stack_top) = (unsigned long )(1<<24); //XPSR 	设置了Thumb模式，恢复到Thumb状态而非ARM状态运行
	*(--stack_top) = (unsigned long )entry;		//程序的入口地址
	*(--stack_top) = (unsigned long )0x14;		//R14(LR)寄存器 任务不会通过return来结束自己，所以没有使用
	*(--stack_top) = (unsigned long )0x12;		//R12 没有使用
	*(--stack_top) = (unsigned long )0x3;		
	*(--stack_top) = (unsigned long )0x2;
	*(--stack_top) = (unsigned long )0x1;
	*(--stack_top) = (unsigned long )param;		//程序传入参数保存在R0寄存器中
	...
	*(--stack_top) = (unsigned long )0x04;
	...
	task->stack = stack_top;   //保存最终的stack值 让该任务的堆栈指针指向栈顶
	

	task->wait_event = (t_event *)0;		//没有等待事件
	task->wait_event_resalt	 = t_error_no_error;
	task->event_msg	= (void *)0;		//没有等待事件的消息存储位置
	
	
	task->slice = TINYOS_SLICE_MAX;		//时间片设为最大
...
void t_get_task_info(tTask * task, t_task_info * info)
{
	...
	info->delay_ticks = task->delay_ticks;				//延时时间
	info->suspend_count = task->suspend_count;		//被挂起次数
	info->prio = task->prio;											//任务优先级
	info->slice	=	task->slice;										//时间片数目
	...
	while((*stack_end++ == 0)&& (stack_end < task->stack_base + task->stack_size / sizeof(tTaskStack)))
	{
		info->stack_free ++;
	}
	
	
	info->stack_free *= sizeof(tTaskStack);
