...
void task1Entry (void *param)
{
	...
	t_flag_group_destroy(&flag_group1);
...
void task2Entry (void *param)
{
	...
	t_flag_group_info group_info;
	//第一次获取到了事件标志
	t_flag_group_wait(&flag_group1, TFLAGGROUP_SET_ALL | TFLAGGROUP_CONSUME, 0x1, &result_flag, 0);
	
	t_flag_group_get_info(&flag_group1, &group_info);
	//第二次应该获取不到了 所以会一直等待 知道任务1把事件标志删除
	t_flag_group_wait(&flag_group1, TFLAGGROUP_SET_ALL | TFLAGGROUP_CONSUME, 0x1, &result_flag, 0);
